...
3. **Start agent** — `startAgent(channelName, rtcUid, userUid)` → `POST /api/startAgent`. Backend validates env vars, builds the `CustomLLM`/`DeepgramSTT`/`MiniMaxTTS` vendor chain, starts the async session, and returns `agent_id`.
4. **Converse** — user audio flows to Deepgram STT (Agora-managed). The transcript is sent by Agora cloud to `CUSTOM_LLM_URL/chat/completions`. The LLM endpoint runs its tool loop and streams the spoken reply. Agora runs MiniMax TTS and plays back the agent's voice. RTM delivers transcript + metrics.
...
- **LLM endpoint owns:** tool execution and SQLite state.
